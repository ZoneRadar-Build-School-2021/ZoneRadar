...
(function () {
  ...
  const readMoreBtn = document.querySelector('.read-more-review');
  ...
  function setReadMoreBtn() {
    const reviewUrl = `/webapi/spaces/GetMoreReview?id=${spaceID}&sentCount=${requestForReviewListObj.sentCount}&addCount=${requestForReviewListObj.addCount}`;
    axios.get(reviewUrl).then(res => {
      let reviewList = res.data.Response;

      spinner.classList.remove('d-none');
      readMoreBtn.classList.add('d-none');
      requestForReviewListObj.sentCount += requestForReviewListObj.addCount;
  ...
  function renderReviews(reviewList = {}) {
    ...
    reviewList.forEach(review => {
      let reviewClone = document.querySelector('#space-review-template').content.cloneNode(true);
      let userAnchor = reviewClone.querySelector('.user-pic');
      let userImgNode = reviewClone.querySelector('img');
      let userNameNode = reviewClone.querySelector('.user-name');
      let starGroupNode = reviewClone.querySelector('.start-group');
      let recommendNode = reviewClone.querySelector('.recommend');
      let reviewContentNode = reviewClone.querySelector('.review-content');
      let reviewTimeNode = reviewClone.querySelector('.review-time');

      // 連結
      userAnchor.setAttribute('href', `~/MemberCenter/Member/${review.UserID}`)
      // 使用者照片
      userImgNode.src = `${review.ReviewedMemberPhoto}`;
      // 使用者名字
      userNameNode.innerText = review.ReviewedMemberName;
      // 評分
      for (let i = 0; i < review.Score; i++) {
        let star = document.createElement('i');
        star.classList = 'fas fa-star';
        starGroupNode.appendChild(star);
      }
      // 是否推薦
      if (review.IsRecommend) {
        recommendNode.innerHTML = '<i class="fas fa-check me-1"></i><span class="recommend-content">我會推薦給其他人</span>'
      } else {
        recommendNode.innerHTML = '<i class="fas fa-times me-1"></i><span class="recommend-content">我不會推薦給其他人</span>'
      }
      // 評論內容
      let content = document.createElement('p');
      content.innerText = review.ReviewContent;
      reviewContentNode.appendChild(content);
      // 評論時間
      reviewTimeNode.innerText = dayjs(review.ReviewDate).format('YYYY/MM/DD');

      reviewNode.appendChild(reviewClone);
      requestForReviewListObj.lastCount = review.NotLoadedCount;
    })

    if (requestForReviewListObj.lastCount > 0) {
  ...
  initialize();
  if (readMoreBtn) {
    readMoreBtn.addEventListener('click', setReadMoreBtn);
  }
  document.querySelector('.read-review').addEventListener('click', function (e) {
    e.preventDefault();
